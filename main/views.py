...
def mylogin(request):
    ...
    if request.method == 'POST':
        ...
        if utxt != "" and ptxt != "":
            ...
            if user != None:
                try:
                    member = Members.objects.get(username=utxt, is_member=True)
                    login(request, user)
                    return redirect('panel')
                    
                except Members.DoesNotExist:
                    
                    msg = "User is found but your are not allowed as staff Please contact with Administrator to get access to this site."
                    return render(request, 'front/msgbox.html', {'msg':msg})
            else:
                return render(request, 'front/msgbox.html', {'msg':"Invalid Username or Password, Please go back to login or create new account"})
